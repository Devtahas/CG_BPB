...
class ClientFrame(ctk.CTkFrame):
    ...
    def start_connection(self):
        ...
        import sys
        # تشخیص هوشمند مسیر: آیا برنامه فایل EXE است یا اسکریپت پایتون؟
        if getattr(sys, 'frozen', False):
            base_dir = os.path.dirname(sys.executable) # مسیر فایل EXE
        else:
            base_dir = os.path.dirname(os.path.abspath(__file__)) # مسیر اسکریپت
